...
FULL_OUT_PATH="${TEMP_DIR}${OUT_FILE_NAME}"
echo "OUT PATH=${FULL_OUT_PATH}"
docker cp $CONTAINER_NAME:/global/zowe/keystore/localhost/localhost.keystore.jwtsecret.p12 $FULL_OUT_PATH
if [ -z "$ZSS_HOST" ]; then
  ZSS_HOST=$(docker exec $CONTAINER_NAME bash -c 'echo $ZOWE_ZSS_HOST')
fi
echo "Initiating scp file transfer to ${ZSS_USER}@${ZSS_HOST}"  
if ! scp $FULL_OUT_PATH $ZSS_USER@$ZSS_HOST:$REMOTE_DIR/$OUT_FILE_NAME ; then
