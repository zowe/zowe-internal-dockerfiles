...
#prevent creation of certificates if they have been created before
if [ "$REGENERATE_CERTIFICATES" == "1" ]; then
  echo "Regenration of certificates enforced, deleteing content of keystore/*"
  rm -rf keystore/*
fi
if [ -d "keystore/local_ca" ]; then
    echo "setup-apiml-certificates.sh has been executed before, so bypassing, in order to rerun certification management, delete local_ca folder " | tee -a $LOG_FILE
    echo "</setup-apiml-certificates.sh>" | tee -a $LOG_FILE
    exit
fi
